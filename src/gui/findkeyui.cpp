...
void FindKeyUi::initUi()
{
    ...
    this->setStyleSheet(QssInstaller::QssInstallFromFile(":/stylesheet/stylesheet_findkeyui.css").arg(this->objectName()).arg("rgb(55,85,100)")
...
//    connect(ui->findKeywordLE, &QLineEdit::textChanged, this, &FindKeyUi::findText, Qt::UniqueConnection);
    ...
    QIcon findPreBtn;
    const QPixmap pixm1 = QPixmap(":/pic/arrow_left_30.png");
    if(!pixm1.isNull()){
    ...
    QIcon findNextBtn;
    const QPixmap pixm2 = QPixmap(":/pic/arrow_right_30.png");
    if(!pixm2.isNull()){
    ...
    QIcon countIcon;
    const QPixmap pixmp1 = QPixmap(":/pic/count_reverse.png");
    if(!pixmp1.isNull()){
